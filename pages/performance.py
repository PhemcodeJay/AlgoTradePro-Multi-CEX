...
# Dynamically import engine and utils based on exchange
if EXCHANGE == "binance":
...
st.set_page_config(
    page_title=f"Performance - AlgoTrader Pro ({EXCHANGE.capitalize()})",
    page_icon="📈",
    layout="wide",
...
def create_equity_curve(trades):
    ...
    try:
        ...
        logger.error(f"Error creating equity curve for {EXCHANGE}: {e}", exc_info=True)
        return None
...
def create_drawdown_chart(trades):
    ...
    try:
        ...
        logger.error(f"Error creating drawdown chart for {EXCHANGE}: {e}", exc_info=True)
        return None
...
def create_returns_distribution(trades):
    ...
    try:
        ...
        logger.error(f"Error creating returns distribution for {EXCHANGE}: {e}", exc_info=True)
        return None
...
def create_performance_metrics_table(trades):
    ...
    try:
        ...
        logger.error(f"Error creating performance metrics for {EXCHANGE}: {e}", exc_info=True)
        return pd.DataFrame()

def main():
    st.markdown(f"""
    <div style="text-align: center; padding: 1rem 0; border-bottom: 2px solid #00ff88; margin-bottom: 2rem;">
        <h1 style="color: #00ff88; margin: 0;">📈 Performance Analytics ({EXCHANGE.capitalize()})</h1>
        <p style="color: #888; margin: 0;">Comprehensive Trading Performance Analysis</p>
    </div>
    """, unsafe_allow_html=True)
    
    try:
        # Initialize engine from session state
        ...
        st.error(f"Error loading performance data for {EXCHANGE}: {e}")
        logger.error(f"Performance page error for {EXCHANGE}: {e}", exc_info=True)
