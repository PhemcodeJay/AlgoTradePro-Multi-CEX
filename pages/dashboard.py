...
# Dynamically import client, engine, and utils based on exchange
if EXCHANGE == "binance":
    from binance_client import BinanceClient
    from multi_trading_engine import TradingEngine
    from binance_ui_utils import (
...
def create_market_overview_chart():
    ...
    try:
        ...
        logger.error(f"Error creating market overview chart for {EXCHANGE}: {e}", exc_info=True)
        return None
...
def create_portfolio_chart(engine):
    ...
    try:
        ...
        logger.error(f"Error creating portfolio chart for {EXCHANGE}: {e}", exc_info=True)
        return None

def load_capital_data(client: Optional[Union['BinanceClient', 'BybitClient']] = None) -> dict:
    """Load capital data from database, syncing real balance if client is connected"""
    # Default fallback
    ...
    try:
        ...
        # Sync real balance with exchange API if client is connected
        if client and client.is_connected():
            try:
                engine = st.session_state.get("engine")
                if engine and engine.sync_real_balance():
                    real_balance_db = db_manager.get_wallet_balance("real")
                    ...
                    if real_balance_db:
                        ...
                        logger.info(f"Real balance synced for {EXCHANGE}: capital=${capital_data['real']['capital']:.2f}, available=${capital_data['real']['available']:.2f}, used=${capital_data['real']['used']:.2f}")
                    else:
                        logger.warning(f"Failed to retrieve real balance after sync for {EXCHANGE}")
                        st.error(f"❌ Failed to retrieve real balance after sync. Check {EXCHANGE} account or API permissions.")
                else:
                    logger.warning(f"Real balance sync failed for {EXCHANGE}")
                    st.error(f"❌ Failed to sync real balance. Check {EXCHANGE} account or API permissions.")
            except Exception as e:
                logger.error(f"Failed to sync real balance from {EXCHANGE}: {e}", exc_info=True)
                st.error(f"❌ Sync failed: {e}")
        else:
            logger.warning(f"{EXCHANGE.capitalize()} client not connected, skipping real balance sync")
        ...
        if capital_data["real"]["available"] == 0.0 and capital_data["real"]["capital"] > 0.0:
            st.info(f"Real available balance is $0.00. Funds may be in use (e.g., open positions).")
        elif capital_data["real"]["available"] == 0.0 and capital_data["real"]["capital"] == 0.0 and client and client.is_connected():
            st.warning(f"No funds available in {EXCHANGE} account. Verify account balance or API permissions.")

        return capital_data

    except Exception as e:
        logger.error(f"Error loading capital data for {EXCHANGE}: {e}", exc_info=True)
        return {
...
def main():
    ...
    <div style="text-align: center; padding: 1rem 0; border-bottom: 2px solid #00ff88; margin-bottom: 2rem;">
        <h1 style="color: #00ff88; margin: 0;">📊 Dashboard ({EXCHANGE.capitalize()})</h1>
    ...
    try:
        ...
        # Load capital data
        capital_data = load_capital_data(client)
        ...
        with tab4:
            ...
            with col3:
                ...
                # Connection status
                connection_status = "✅ Connected" if client and client.is_connected() else "❌ Disconnected"
        ...
        st.error(f"Dashboard error for {EXCHANGE}: {e}")
