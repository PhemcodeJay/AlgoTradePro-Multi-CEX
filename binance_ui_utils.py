...
def normalize_signal(signal: Any) -> Dict:
    """Normalize signal data to a consistent dictionary format"""
...
def display_trades_table(trades: List[Dict], container, client=None, max_trades: int = 10):
    """Display trades in a formatted table using Streamlit"""
    try:
        if not trades:
...
def display_log_stats(log_file: str, container, refresh_key: str):
    """Display log statistics and recent logs in Streamlit"""
    try:
...
def get_trades_safe(db_instance, limit: int = 50) -> List[Dict]:
    """Safely retrieve trades from database"""
    try:
...
def get_open_trades_safe(db_instance) -> List[Dict]:
    """Safely retrieve open trades from database"""
    try:
...
def get_signals_safe(db_instance, limit: int = 20) -> List[Dict]:
    """Safely retrieve signals from database"""
    try:
...
def get_ticker_snapshot() -> List[Dict]:
    """Fetch all USDT tickers with last price and 24h change from Binance"""
    try:
        ...
        usdt_tickers = [
            {
                ...
                "price24hPcnt": float(ticker["priceChangePercent"]) / 100,
                "volume24h": ticker["volume"],
                "highPrice24h": ticker["highPrice"],
                "lowPrice24h": ticker["lowPrice"]
            }
            for ticker in data if ticker.get("symbol", "").endswith("USDT")
        ]
        return usdt_tickers
    except Exception as e:
...
def sync_real_wallet_balance(client, update_file: bool = True) -> Dict[str, Any]:
    """Sync real wallet balance with Binance and optionally update capital.json"""
    try:
        if not client or not client.is_connected():
            logger.warning("Binance client not connected")
            return {}
        ...
        balance_data = {
            "capital": usdt_balance.capital if usdt_balance else 0,
            "available": usdt_balance.available if usdt_balance else 0,
        ...
        if update_file:
            try:
                ...
                logger.info("Real balance synced with Binance")
            except Exception as e:
